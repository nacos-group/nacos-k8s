...
func (e *KindClient) buildStatefulsetCluster(nacos *nacosgroupv1alpha1.Nacos, ss *appv1.StatefulSet) *appv1.StatefulSet {
	...
	// 先检查域名解析再启动
	ss.Spec.Template.Spec.Containers[0].Command = []string{"bash", "-c", fmt.Sprintf("%s&&bin/docker-startup.sh", fmt.Sprintf(initScript, serivceNoPort))}
	return ss
}
